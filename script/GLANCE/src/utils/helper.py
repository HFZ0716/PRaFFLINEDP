...
from ..utils.config import PROJECT_RELEASE_LIST
...
file_level_path = f'{root_path}/{dataset_string}/original_newData/File-level/'
line_level_path = f'{root_path}/{dataset_string}/original_newData/Line-level/'
...
def read_line_level_dataset(release=''):
    ...
    with open(path, 'r', encoding='utf-8', errors='ignore') as file:
        ...
        for line in lines[1:]:
            temp = line.split(',', 2)
            # print("buggy_line_number", int(temp[1].strip('"')))
            file_name, buggy_line_number = temp[0], int(temp[1].strip('"'))
