...
class LineDP(BaseModel):
    ...
    def line_level_prediction(self):
        ...
        for i in range(len(defective_file_index)):
            ...
            # 检查文件内容是否有效
            processed_text = ' '.join(defective_file_line_list).strip()
            if not processed_text:
                print(f"警告: 文件索引 {defective_file_index[i]} 内容为空，跳过处理")
                continue  # 跳过当前文件
            ...
            try:
                ...
                if "low >= high" in str(e):
                    print(f"警告: 文件索引 {defective_file_index[i]} 生成解释失败，跳过处理")
                else:
                    raise  # 其他类型错误继续抛出
