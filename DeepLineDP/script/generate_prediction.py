...
save_model_dir = 'Baseline-result/DeepLineDP/output/model/DeepLineDP/'
intermediate_output_dir = 'Baseline-result/DeepLineDP/output/intermediate_output/DeepLineDP/within-release/'
        ...
        for filename, df in tqdm(test_df.groupby('filename')):
            ...
            if not os.path.exists(save_file_path):
                with torch.no_grad():
                    ...
                    codevec_padded_tensor = torch.tensor(codevec).contiguous()
                    ...
                    try:
                        pickle.dump(output_dict, open(absolute_save_file_path, 'wb'))
