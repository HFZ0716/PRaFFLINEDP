# -*- coding:utf-8 -*-
import numpy as np
from ..utils.config import USE_CACHE
...
# 定义了一个名为Glance的类，它是所有基于Glance的方法的基础类。它重写了line_level_prediction方法。
...
# 基于Glance的类：定义了一系列基于Glance的方法，如Glance_MD、Glance_MD_full_threshold、Glance_EA等。每个类都重写了file_level_prediction方法，用于文件级别的缺陷预测
...
# Overwrite method file_level_prediction(), which is unique for different Glance based approaches.
# 基于Glance的类：Glance_MD_full_threshold类都重写了file_level_prediction方法，用于文件级别的缺陷预测
class Glance_MD_full_threshold(Glance):
    ...
    def __init__(self, train_release='', test_release='', line_threshold = 0.5, file_threshold = 1, test=False,
                 is_realistic=False):
        test_result_path = ''
        if test:
            self.model_name = f'Glance-MD-{str(int(file_threshold * 100))}-{str(int(line_threshold * 100))}'
...
# 基于Glance的类：Glance_EA类都重写了file_level_prediction方法，用于文件级别的缺陷预测
...
class Glance_LR(Glance):
    ...
    def __init__(self, train_release='', test_release='', line_threshold=0.5, file_threshold = 1,test=False, is_realistic=False):
        test_result_path = ''
        if test:
    ...
    def file_level_prediction(self):
        ...
        # 在 src/models/glance.py 第392行之前（即调用fit_transform前）
